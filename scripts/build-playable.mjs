...
const cutValue = top[TOP - 1].decks;
const tied = ranked.filter((r) => r.decks === cutValue).length;
const tiedIn = top.filter((r) => r.decks === cutValue).length;
...
const rows = top
  ...
  <span class="pl-body">
    ...
    ${/* "LISTS" FOR DECKLISTS, "DECKS" FOR ARCHETYPES, and never the other way
         ...
         the same reason. */ ""}
    <span class="pl-nums">
      <b>${r.decks}</b> of ${lists} lists <span class="pl-pctv">(${pct(r.decks)}%)</span>
  ...
  <section class="tight">
    <div class="wrap">
      ...
      <p class="pl-note"><b>Two things this count is not.</b> Every deck contributes about the same number
        ...
        ${corpus.listsPerArchetype} lists, a card that is a staple of exactly one deck lands on
        ${cutValue}: <b>${tied} cards tie on ${cutValue} decks</b>, of which ${tiedIn} fit inside the
        hundred. The cut at 100 is arbitrary between those, so read the bottom of this list as a group and
        not as an order.</p>
...
console.log(
  `  top ${TOP} of ${ranked.length} distinct cards over ${lists} lists; ` +
    `${noArt} without a scan; tie at the cut: ${tied} cards on ${cutValue} decks`
);
console.log(`  #1 ${top[0].name} (${top[0].decks} decks, ${top[0].copies} copies)`);
