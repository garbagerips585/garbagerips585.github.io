...
/* THIS WAS \`@media(max-width:360px){.e404-mascot{display:none}}\` AND HIDING HIM
   TURNED THE JOKE INTO A BROKEN IMAGE. The pack is deliberately the LEFT half of
   a torn one: .pack-l clips it at about 53%, which is why the rule above pulls
   the box back 38% to sit the mascot in the gap. Take the mascot away and all
   that is left is a booster pack ending in a clean vertical line through "Poke"
   and "TRADING C", with 38% of its own box empty beside it. At 390 that reads as
   a pack coming open with Trubbish looking into it. At 320 it reads as a
   photograph that failed to finish loading, on the one page whose whole job is
   to reassure somebody that nothing is broken.

   THE ROOM WAS ALWAYS THERE, and that is the part that makes this a deletion
   rather than a redesign. Measured at 320x900: .e404-in is 288px wide and both
   clamps are already on their floors at that width, so the pair draws
   148 - (148 x .38) + 104 = 195.8px of art, 68% of the box, against 209.8 of 358
   at 390. Nothing overflowed and nothing ever would have. Re-measured after:
   scrollX 0 and no element past the right edge at 320, 360 and 390. */
