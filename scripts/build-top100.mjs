...
/* TWO COLUMNS FROM 700 AND NOT 900, BECAUSE 900 PUT AN iPAD ON THE PHONE
   LAYOUT. An iPad in portrait is 768 CSS px, below the old 900, so a tablet
   showed these four notes as 720px-wide boxes: a phone layout at double width.
   700 is measured rather than picked: the wrap is 660px inside a 700px viewport,
   which is two 325px boxes and the 10px gap, and at 390 the wrap is 350px where
   one column is still the only fit. Measured at 768, one column to two: 640px
   to 535, a 16% shorter methodology block.
   auto-fit WAS TRIED HERE AND IT IS WRONG, WHICH IS WHY THIS SAYS 1fr 1fr.
   This is the only column rule on the grid, so whatever it says also governs
   1440. repeat(auto-fit,minmax(min(320px,100%),1fr)) gives the two columns that
   were wanted at 768, and then FOUR columns of 341px in the 1,392px wrap at
   1440, which is not a layout anybody chose for four methodology notes: it
   shreds a 499 character paragraph into a 341px ribbon. Measured, not guessed.
   A floor big enough to refuse the fourth column at 1440 is over 460px, and
   that floor refuses the SECOND column at 768, which is the bug this rule
   exists to fix. There is no floor that satisfies both, so the count is written
   down instead. If a fifth note is ever added, this is the line to revisit. */
@media (min-width:700px){.t100-src{grid-template-columns:1fr 1fr}}
`;
